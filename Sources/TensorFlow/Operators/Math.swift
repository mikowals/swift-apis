...
public func gelu<T: TensorFlowFloatingPoint>(_ x: Tensor<T>) -> Tensor<T> {
  ...
  let ratio1 = Tensor<T>(0.7978845608, deviceAndPrecisionLike: y)  // An approximation of √(2/π).
  ...
  let ratio2 = Tensor<T>(0.044715, deviceAndPrecisionLike: y)
  let half = Tensor<T>(0.5, deviceAndPrecisionLike: y)
  let one = Tensor<T>(1, deviceAndPrecisionLike: y)
  let three = Tensor<T>(3, deviceAndPrecisionLike: y)
